...
const BallRunnerGame: React.FC = () => {
  ...
  const [ballPosition, setBallPosition] = useState<Position>({
    x: screenWidth / 2,
    y: screenHeight - 400, // Start higher than before
  });
  ...
  const loadSavedData = async (): Promise<void> => {
    try {
      ...
      const savedHighScore = await AsyncStorage.getItem(
  ...
  const checkCollisions = (): void => {
    ...
    collectibleBallsRef.current.forEach(
      (ball: CollectibleBall, index: number) => {
        if (!ball.collected) {
          const ballLeft: number = ballRef.current.x - BALL_SIZE / 2;
          const ballRight: number = ballRef.current.x + BALL_SIZE / 2;
          const ballTop: number = ballRef.current.y - BALL_SIZE / 2;
  ...
  const gameLoop = (): void => {
    ...
    // Remove old items
    boxesRef.current = boxesRef.current.filter(
      (box: Box) => box.x > cameraRef.current - 100
    );
  ...
  const renderGame = (): JSX.Element => (
    <View style={styles.gameContainer} {...panResponder.panHandlers}>
      ...
      <View style={styles.scoreContainer}>
        ...
        {highScore && (
          <Text style={styles.highScoreInGame}>High: {highScore.score}</Text>
        )}
      </View>
    ...
    <View style={styles.nameInputContainer}>
      ...
      <View style={styles.howToPlayContainer}>
        ...
        <Text style={styles.howToPlayText}>
          ...
          {"\n"}• Avoid red triangles (appear after 500 points) - they end the
          game!
